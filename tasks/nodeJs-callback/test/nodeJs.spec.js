...
describe("JSON Parser", () => {
  ...
  before((done) => jsonParser(done));
  beforeEach(() => (parserJson = require("../parser/parsed.json")));

  it(".html should be replaced for every item", () => {
    expect(parserJson.every((item) => item.docId.includes("html"))).to.equal(
      false
    );
  });
  ...
  it("every item should contain http://doc.epam.com/ string", () => {
    expect(
      parserJson.every((item) => item.docId.includes("http://doc.epam.com/"))
    ).to.equal(true);
  });
});
